...
*   
*   5.20 Expanded: Expand dcl to handle declarations with function arguments types, qualifiers like const, and so on. Include functionality 
*   for modifiers.
*   
*   Notes: This has the same constraints as 5.20, only now a modifers are allowed to prefix the declaration. 
*********************************************************************************************************************************************/
#include <stdio.h>
#include <string.h>
#include <ctype.h>
...
enum{NAME, TYPE, QUALIFIER, MODIFIER, PARENS, BRACKETS};
...
void obtain_return_type(char* hold, char* modifier_hold);
int gettoken(void); 

int tokentype;                      /* type of last token */
char token[MAXTOKEN];               /* last token string */
char name_master[MAXTOKEN];         /* identifer name */
char modifier_master[MAXTOKEN];     /* modifier name */
...
int main()
{
    ...
    while (gettoken() != EOF){
        obtain_return_type(datatype_master, modifier_master); /* obtains the type */
        ...
        strcat(final_output, modifier_master);
        strcat(final_output, ".");
...
void obtain_return_type(char* hold, char* modifier_hold)
{
    ...
    if (tokentype == MODIFIER){
        strcpy(modifier_hold, " stored as ");
        strcat(modifier_hold, token );
        gettoken();
    }
